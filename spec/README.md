...
- Surface Prompt에는 제품명/제품군 키워드를 넣지 않고, 합성 준비용 배경 장면 규칙을 우선한다
...
4. `PRODUCT_IN_SCENE`에서도 제품 직접 지칭 대신 배치면/여백/스케일 규칙을 우선 출력한다.
5. 스케일은 대상 물체가 아닌 **지지 가구(테이블/선반/협탁) 치수 기준**으로 해석한다.
6. master ratio는 `{1:1, 3:2}`만 허용한다.
